<template>
  <div class="report-wrap-module elevator-module" :style="{top: navbarOffsetTop + 'px'}" :class="{ 'edit-state' : isSort }">
    ...
    <div class="navbar-list" ref="list">
      <template v-for="(item, index) in floorData">
        <div
          class="item sortable"
          :key="item.id"
          v-if="isDrag && index === replaceItem && replaceItem <= dragId"></div>
        <div
          class="item sotrable"
      ...
      <div class="item customize" @click="activeSort"><i class="icon"></i>排序</div>
    </div>
    <div class="s-line"></div>
    <div class="back-top icon" @click="scrollToTop(time)"></div>
  </div>
</template>
...
export default {
  ...
  data () {
    return {
      ...
      navbarOffsetTop: this.options.offsetTop, // navbar距离页面顶部的距离
      time: 500, // 滚动缓动动画时间
  ...
  methods: {
    ...
    initData () {
      ...
      this.floorData = Array.from(this.options.items, item => {
        const element = document.getElementById('bili_' + item.category)
        if (!element) {
    ...
    scroll  (e) {
      ...
      for (let i = 0; i < this.floorData.length; i++) {
        ...
        this.scrollTop >= this.offset
          ? (this.navbarOffsetTop = 0)
          : (this.navbarOffsetTop = this.options.offsetTop)
      }
    },
    scrollToFloor  (index) {
      // 缓动滚动到指定楼层
  ...
  computed: {
    ...
    replaceItem () {
      ...
      if (id > this.floorData.length - 1) id = this.floorData.length
      if (id < 0) id = 0
      return id
    }
  },
...
.elevator-module
  ...
  .nav-bg
    ...
    .tips-img
      ...
      width 117px
      height 333px
      background url('~assets/images/base/tab2233.png')
      left 12px
      top 14px
</style>
